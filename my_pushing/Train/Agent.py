...
class DDPGAgent(Agent):
    ...
    def __init__(self, env: Env = None,
        ...
        self.state_dim = env.observation_space.shape[0] if not state_dim else state_dim# 状态连续
        self.action_dim = env.action_space.shape[0] if not action_dim else action_dim# 行为连续
    ...
    def _learn_from_memory(self):
        ...
        y_expected = torch.tensor(r1).to(device) + self.gamma * next_val
        y_expected = y_expected.type(torch.float).to(device)
        # y_pred = Q( s1, a1)
    ...
    def learning_method(self, display=True, explore=True):
        # 状态用一个x,y,z表示6元素，分别代表物体当前位置和目标位置
        state = self.env.reset()

        self.state = np.float64(np.append(np.append(state['achieved_goal'], state['desired_goal']), state['observation']))
